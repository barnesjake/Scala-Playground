...
object Artist {
  val availableArtist: Seq[Artist] = Seq(
  ...
  def fetchByName(name: String): Seq[Artist] = availableArtist.filter(a => a.name.contains(name))

  def fetchByCountry(country: String): Seq[Artist] = availableArtist.filter(a => a.country == country)
